...
function solution(X, A) {
    ...
    var counter = new Array(X);

    var starTime = 0;

    console.log('counter inited', counter);

    A.forEach(function(a, index) {
      console.log('parsing element [', index, '] = ',a);
      if (!counter[a]) {
        counter[a] = 0;
      }
      counter[a] += 1;
      ...
      var goFrogGo = counter.every(function(elm) {
        return elm > 0;
      });

      if(goFrogGo) {
        console.log('Fron can go', goFrogGo, ' at ', index-1);
        starTime = index - 1;
      };
